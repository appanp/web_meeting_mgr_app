...
}).register("evg_load_template",function(){var i=d.personalize||[{}],j=i[i.length-1];
j.event="custEv_personalize";
return j
})
}());
"use strict";
...
(function(){function b(){try{window.custom_var=Page.getLocale()+"|"+vp.getType()+"|"+Page.getSegment()+"|"+vp.getTypeDetailed()+"|"+vp.getVisitNumber()+"|"+Page.getName()
}catch(d){}}function c(d){if(!d.detail||!d.detail.payload){return
}var g=d.detail.payload,f={personalizeCampaign:g.campaign||"",personalizeExperience:g.experience||"",personalizeUserGroup:g.userGroup||""};
if(!digitalData.page.personalize){digitalData.util.addPageData("personalize",[f])
}else{digitalData.page.personalize.push(f)
}digitalData.util.trackBehavior("evg-load-template",["evg_load_template"],["personalize"])
}function a(){if(!digitalData||!digitalData.util){return
}document.addEventListener("evergage:onTemplateDisplayEnd",c)
}digitalData.util.components.addIntegrations=function(){b();
a()
}
}());
...
var h=function(){var k="social:twitter",l="social:Facebook",n="social:LinkedIn",m="social:Youtube",j="social:Instagram",o="social:Google";
document.addEventListener("click",function(u){var v="undefined";
if(u.target.matches("footer.bottom a")){var t=u.target,r=t.innerText.trim().toLowerCase(),w=t.closest(".genericLinkListComponent")?t.closest(".genericLinkListComponent").previousElementSibling:"",x=w?w.innerText.trim().toLowerCase():(t.closest(".page-footer_content")?"legal and privacy":"");
v=x?x+":"+r:r
}if(u.target.matches("footer .footer-logo a")||u.target.matches("footer .footer-logo a img")){v="home:logo"
}var p=u.target,q=p.classList;
if(p.getAttribute("src")&&q&&q.contains("salesforce-social-icon")){var y=p.getAttribute("src");
if(y.includes("linkedin")){v=n
}else{if(y.includes("twitter")){v=k
}else{if(y.includes("facebook")){v=l
}else{if(y.includes("youtube")){v=m
}else{if(y.includes("instagram")){v=j
}else{if(y.includes("google")){v=o
}}}}}}}if(v!=="undefined"){b.addPageData("navItem",v);
...
}const a={LISTVAR:"oe_experiments",campaignArray:[],assignCampaigns:function(b){b[this.LISTVAR]=b[this.LISTVAR]||[];
for(var c=this.campaignArray.length-1;
c>=0;
c--){b[this.LISTVAR].push(this.campaignArray[c]);
this.campaignArray.splice(c,1)
}},trackDelayedCampaigns:function(b){b[this.LISTVAR]=b[this.LISTVAR]||[];
b.linkTrackVars+=(","+this.LISTVAR);
for(var c=this.campaignArray.length-1;
c>=0;
c--){b[this.LISTVAR].push(this.campaignArray[c]);
this.campaignArray.splice(c,1)
}b.tl(true,"o","OptimizelyLayerDecision")
},formatActiveExperiments:function(){let expList=[],expObj=window.optimizelyEdge.get("state").getActiveExperiments();
for(var b in expObj){expList.push(expObj[b].id+":"+expObj[b].variation.id)
}return expList
},init:function(){let activeExperimentsArray=this.formatActiveExperiments();
if(activeExperimentsArray.length){this.campaignArray=activeExperimentsArray
}}};
a.init();
window.getDigitalDataIntegrator=function(b){if(b==="adobe"){return a
}throw new Error(b+" is not a supported module")
}
})();
(function(){if(!window.optimizelyEdge){return
}window.optlyHelper=(function(){return{v:"1.26",getCookieValue:function(d){var c=document.cookie.match("(^|[^;]+)\\s*"+d+"\\s*=\\s*([^;]+)");
return c?c.pop():""
},setCookieValue:function(f,i,g){var h=new Date();
h.setTime(h.getTime()+(g*24*60*60*1000));
var c="expires="+h.toUTCString();
document.cookie=f+"="+i+";"+c+";domain=.{salesforce}.com;path=/"
},activeExperiments:{}}
})();
const a=(function(){let activeExperiments,redirectCookie,redirectVariables;
function g(i){var h=document.cookie.match("(^|[^;]+)\\s*"+i+"\\s*=\\s*([^;]+)");
return h?h.pop():""
}function c(m){var k={};
var j=(m[0]==="?"?m.substr(1):m).split("&");
for(var h=0;
h<j.length;
h++){var l=j[h].split("=");
k[decodeURIComponent(l[0])]=decodeURIComponent(l[1]||"")
}return k
}function d(h){optlyHelper.redirectInfo=h
}function f(){activeExperiments=window.optimizelyEdge.get("state").getActiveExperiments();
redirectCookie=g("optimizelyRedirectData");
...
const b=(function(){var f=100,d=10000,h=0,i;
function c(m){if(m){var l="tealium_0.";
window.ga(l+"send","event","Optimizely","Assigned to Experiment",m,{nonInteraction:true})
}}function g(l){if(window.ga&&window.ga.getByName){l()
}else{if(h<d){setTimeout(function(){h=h+f;
g(l)
},f)
}}}function k(l){return"("+l.id+":"+l.variation.id+")"
}function j(){g(function(){i=optlyHelper.activeExperiments;
for(var m in i){if(i.hasOwnProperty(m)){var l=i[m];
var n=k(l);
c(n)
}}})
}return{init:j,waitForGaEventApi:g}
})();
a.init();
b.init()
})();